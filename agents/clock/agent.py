"""
Clock Agent
Returns the current date and time for any timezone via the A2A protocol.
Default port: 9001
"""
...
def run(port: int = PORT):
    ...
    agent = Agent(
        name=NAME,
        description="A clock agent that returns the current date and time for any timezone.",
        model=llm_model,
        tools=[current_time],
